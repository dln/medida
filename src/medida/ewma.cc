...
namespace medida {
  ...
  EWMA EWMA::oneMinuteEWMA() {
    return EWMA(kM1_ALPHA, std::chrono::seconds(5));
  }

  EWMA EWMA::fiveMinuteEWMA() {
    return EWMA(kM5_ALPHA, std::chrono::seconds(5));
  }

  EWMA EWMA::fifteenMinuteEWMA() {
    return EWMA(kM15_ALPHA, std::chrono::seconds(5));
  }
  ...
  void EWMA::tick() {
    ...
    double instantRate = count / interval_.count();
    if (initialized_) {
